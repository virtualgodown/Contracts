...
function presale_1(address _from, uint256 _value) internal {
  ...
  //if ((totalTokensIssuedInPresaleOne + rimule) >= hardCap) revert(); //if presale value reaches hardcap , reverts
  else {
...
//presale2
  function presale_2(address _from, uint256 _value) internal {
    ...
    if (mintedToken < rimule) revert();
    //if ((totalTokensIssuedInPresaleTwo + rimule) >= hardCap) revert(); //if presale value reaches hardcap , reverts
    else {
